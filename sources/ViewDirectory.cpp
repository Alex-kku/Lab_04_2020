...
ViewDirectory::ViewDirectory(const std::string& path) {
  if (!path.empty()) {
    if (fs::exists(path)) {
      pathFTP = path;
    }
...
std::stringstream ViewDirectory::PrintFiles() {
  std::stringstream ss;
...
std::stringstream ViewDirectory::PrintAccounts() const {
  std::stringstream ss;
  for (const auto& x : accounts) {
    ...
    for (const auto& y : paths) {
      if (x == y.stem().string().substr(AccountPosition, PositionLength)) {
        if ((n < 1) ||
            (y.stem().string().substr(TimePosition, PositionLength) >
